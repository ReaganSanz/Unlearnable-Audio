...
class PerturbationTool:
    ...
    def min_min_attack(self, audio_samples, labels, model, optimizer, criterion, random_noise=None, sample_wise=False):
        ...
        perturb_audio = Variable(torch.clamp(perturb_audio, -1, 1), requires_grad=True) # was -1 and 1. Do I need to normalize?
        eta = random_noise
        # Go through num_steps times. 
        for _ in range(self.num_steps):
            ...
            loss.backward()
            # Update perturbations (minimize loss)
            eta = self.step_size * perturb_audio.grad.data.sign() * (-1) # take a step of size step_size that maximizes loss/gradient decent
