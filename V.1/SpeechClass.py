...
# Contains names of all 32 sound labels
...
def label_to_index(word):
   ...
   # print(f"Labels: {labels}, Type[0]: {type(labels[0])}", flush=True)
...
def collate_fn(batch):
    ...
    #DEBUG
   # print(f"After padding: tensors size: {tensors.size()}, targets size: {targets.size()}", flush=True)
...
#### ADDED CRITERION and random noise####
...
target_accuracy_rate = 50.0 # accuracy threshold? (VERY low right now for testing)
eps = 0.03137254901960784   # epsolon
...
while condition:
    ...
    print(f"Condition: {100-error_rate * 100}...(error) > {target_accuracy_rate}(target). Loss_avg = {loss_avg}")
   # if error_rate < target_error_rate:
   #     condition = False

    # Chceck if accuracy has reached threshold (Might need updating)
    if (100-error_rate * 100) > target_accuracy_rate:
        condition = False
## END OF CONDITION LOOP

## NOTE: Anything Beyond this section has not been tested
